...
def assert_json_response(response: requests.Response) -> Dict[str, Any]:
    ...
    try:
        ...
        raise AssertionError(
            f"Response is not valid JSON: {str(e)}. Body: {response.text}"
        )
...
def assert_iso_datetime_format(value: str, field_name: str = "timestamp"):
    ...
    try:
        ...
        raise AssertionError(
            f"Field '{field_name}' is not a valid ISO datetime: {value}"
        )
...
def assert_snapshot_response_schema(data: Dict[str, Any]):
    ...
    required_fields = ["dataset_id", "version_name", "files_summary", "data"]
    assert_response_schema(data, required_fields)

    # Validate field types
    assert_uuid_format(data["dataset_id"], "dataset_id")
    assert isinstance(
        data["version_name"], str
    ), f"version_name should be str, got {type(data['version_name'])}"
    assert isinstance(
        data["data"], dict
    ), f"data should be dict, got {type(data['data'])}"

    # Optional fields validation
    if "doi_identifier" in data and data["doi_identifier"] is not None:
        assert isinstance(
            data["doi_identifier"], str
        ), f"doi_identifier should be str or None, got {type(data['doi_identifier'])}"

    if "doi_link" in data and data["doi_link"] is not None:
        assert isinstance(
            data["doi_link"], str
        ), f"doi_link should be str or None, got {type(data['doi_link'])}"
        # Basic URL validation
        if not (
            data["doi_link"].startswith("http://")
            or data["doi_link"].startswith("https://")
        ):
            raise AssertionError(
                f"doi_link should be a valid URL, got: {data['doi_link']}"
            )

    if "doi_state" in data and data["doi_state"] is not None:
        assert isinstance(
            data["doi_state"], str
        ), f"doi_state should be str or None, got {type(data['doi_state'])}"
        ...
        if data["doi_state"].lower() not in valid_doi_states:
            raise AssertionError(
                f"doi_state should be one of {valid_doi_states}, got: {data['doi_state']}"
            )
...
def assert_files_summary_schema(files_summary: Dict[str, Any]):
    ...
    # Validate field types
    assert isinstance(
        files_summary["total_files"], int
    ), f"total_files should be int, got {type(files_summary['total_files'])}"
    assert isinstance(
        files_summary["total_size_bytes"], int
    ), f"total_size_bytes should be int, got {type(files_summary['total_size_bytes'])}"
    assert isinstance(
        files_summary["extensions_breakdown"], list
    ), f"extensions_breakdown should be list, got {type(files_summary['extensions_breakdown'])}"

    # Validate non-negative values
    if files_summary["total_files"] < 0:
        raise AssertionError(
            f"total_files should be non-negative, got: {files_summary['total_files']}"
        )
    if files_summary["total_size_bytes"] < 0:
        raise AssertionError(
            f"total_size_bytes should be non-negative, got: {files_summary['total_size_bytes']}"
        )
...
def assert_file_extension_summary_schema(
    ext_summary: Dict[str, Any], context: str = "extension_summary"
):
    ...
    # Validate field types
    assert isinstance(
        ext_summary["extension"], str
    ), f"{context}: extension should be str, got {type(ext_summary['extension'])}"
    assert isinstance(
        ext_summary["count"], int
    ), f"{context}: count should be int, got {type(ext_summary['count'])}"
    assert isinstance(
        ext_summary["total_size_bytes"], int
    ), f"{context}: total_size_bytes should be int, got {type(ext_summary['total_size_bytes'])}"

    # Validate non-negative values
    if ext_summary["count"] < 0:
        raise AssertionError(
            f"{context}: count should be non-negative, got: {ext_summary['count']}"
        )
    if ext_summary["total_size_bytes"] < 0:
        raise AssertionError(
            f"{context}: total_size_bytes should be non-negative, got: {ext_summary['total_size_bytes']}"
        )

    # Validate extension format (should start with dot)
    if not ext_summary["extension"].startswith("."):
        raise AssertionError(
            f"{context}: extension should start with '.', got: {ext_summary['extension']}"
        )


def assert_dataset_version_info_schema(
    version: Dict[str, Any], context: str = "version"
):
    ...
    assert isinstance(
        version["id"], str
    ), f"{context}: id should be str, got {type(version['id'])}"
    assert isinstance(
        version["name"], str
    ), f"{context}: name should be str, got {type(version['name'])}"

    # Optional fields validation
    if "doi_identifier" in version and version["doi_identifier"] is not None:
        assert isinstance(
            version["doi_identifier"], str
        ), f"{context}: doi_identifier should be str or None, got {type(version['doi_identifier'])}"

    if "doi_state" in version and version["doi_state"] is not None:
        assert isinstance(
            version["doi_state"], str
        ), f"{context}: doi_state should be str or None, got {type(version['doi_state'])}"
        valid_doi_states = ["draft", "registered", "findable"]
        if version["doi_state"].lower() not in valid_doi_states:
            raise AssertionError(
                f"{context}: doi_state should be one of {valid_doi_states}, got: {version['doi_state']}"
            )
...
def assert_error_response_schema(data: Dict[str, Any]):
    ...
    assert isinstance(
        data["detail"], str
    ), f"detail should be str, got {type(data['detail'])}"
...
def assert_fastapi_validation_error_schema(data: Dict[str, Any]):
    ...
    if "detail" not in data:
        raise AssertionError(
            f"Validation error response missing 'detail' field. Data: {data}"
        )
    ...
    if isinstance(detail, list):
        for i, error in enumerate(detail):
            if not isinstance(error, dict):
                raise AssertionError(
                    f"Validation error detail[{i}] should be dict, got {type(error)}"
                )

            # Validate error object structure
            if "loc" not in error or "msg" not in error or "type" not in error:
                raise AssertionError(
                    f"Validation error detail[{i}] missing required fields (loc, msg, type). Got: {error}"
                )
        ...
        raise AssertionError(
            f"Validation error detail should be list or str, got {type(detail)}"
        )


def assert_response_matches_dict(
    response: requests.Response, expected_dict: Dict[str, Any]
):
...
def assert_response_matches_json_string(
    response: requests.Response, expected_json_string: str
):
...
def assert_response_contains_fields(
    response: requests.Response, expected_fields: Dict[str, Any]
):
    ...
    for field, expected_value in expected_fields.items():
        if field not in actual_data:
            raise AssertionError(
                f"Response missing field '{field}'. Response: {actual_data}"
            )
