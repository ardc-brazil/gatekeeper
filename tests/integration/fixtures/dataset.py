...
class DatasetFixture:
    ...
    def create_test_dataset(self) -> Dict[str, Any]:
        ...
        if response.status_code != 201:
            raise Exception(f"Failed to create test dataset: {response.status_code} - {response.text}")
    ...
    def create_dataset_with_version(self) -> Dict[str, Any]:
        ...
        if response.status_code != 201:
            raise Exception(f"Failed to create dataset version: {response.status_code} - {response.text}")
    ...
    def create_published_dataset_with_snapshot(self) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(f"Failed to enable dataset version: {response.status_code} - {response.text}")
