...
class DatasetPendingCollocationResponse(BaseModel):
    ...
    created_at: datetime = Field(..., title="Created at")
    file_collocation_status: Optional[str] = Field(None, title="File collocation status")
...
class UpdateFilePathRequest(BaseModel):
    ...
    storage_path: str = Field(..., title="New storage path", min_length=1, max_length=2048)
