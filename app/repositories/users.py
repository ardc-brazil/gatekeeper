...
class UsersRepository:
    ...
    def fetch_by_provider(self, provider_name, reference, is_enabled=True):
        ...
        user = db.session.query(Users). \
            ...
            join(provider_alias, provider_alias.id == user_provider_association.c.provider_id). \
            filter(provider_alias.name==provider_name, Providers.reference==reference, Users.is_enabled==is_enabled). \
            first()
